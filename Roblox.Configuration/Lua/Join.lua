...
pcall(function()
	...
	game:HttpGet("_burl/Game/JoinRate.ashx?st=0&i=0&p=-1&c=GameAppInit&r=Success&d=" .. (math.floor(t*1000)) .. "&ip=localhost&errorType=&platform=" .. platform, false)
end)

pcall(function() game:SetPlaceID(_pid, false) end)
...
function reportContentProvider(time, queueLength, blocking)
	pcall(function()
		game:HttpGet("_burl/Analytics/ContentProvider.ashx?t=" .. time .. "&ql=" .. queueLength, blocking and not isTouchDevice)
	end)
end
function reportCdn(blocking)
	pcall(function()
		...
		if successDelta > 0 or failureDelta > 0 then
			game:HttpGet("_burl/Game/Cdn.ashx?source=client&success=" .. successDelta .. "&failure=" .. failureDelta, blocking and not isTouchDevice)
		end
	end)
end
...
function reportDuration(category, result, duration, blocking,errorType)
	...
	pcall(function() game:HttpGet("_burl/Game/JoinRate.ashx?st=0&i=0&p=-1&c=" .. category .. "&r=" .. result .. "&d=" .. (math.floor(duration*1000)) .. "&b=" .. bytesReceived .. "&ip=localhost&errorType=" .. errorType .. "&platform=" .. platform, blocking and not isTouchDevice) end)
end
-- arguments ---------------------------------------
...
local closeConnection = game.Close:connect(function() 
	if 0 then
		...
		if true then pcall(function() game:HttpPost("_aurl/auth/invalidate", "invalidate") end) end
	end
end)
...
pcall(function() game:SetCreatorID(_cid, Enum.CreatorType.User) end)
...
local success, err = pcall(function()	
	...
	playerConnectSucces, player = pcall(function() return client:PlayerConnect(0, "_host", _port, 0, threadSleepTime) end)
	if not playerConnectSucces then
		--Old player connection scheme
		player = game:GetService("Players"):CreateLocalPlayer(0)
		analytics("Created Player")
		client:Connect("_host", _port, 0, threadSleepTime)
	else
		analytics("Created Player")
	end
	...
	if true then
		pcall(function() game:HttpPost("_aurl/auth/negotiate?ticket=", "negotiate") end)
		delay(300, function()
			while true do
				pcall(function() game:HttpPost("_aurl/auth/renew", "renew") end)
				wait(300)
			end
		end)
	end

	player:SetSuperSafeChat(_schat)
	pcall(function() player:SetUnder13(false) end)
	pcall(function() player:SetMembershipType(Enum.MembershipType._mem) end)
	pcall(function() player:SetAccountAge(_age) end)
	player.Idled:connect(onPlayerIdled)
	
	-- Overriden
	onPlayerAdded(player)
	
	pcall(function() player.Name = [========[_name]========] end)
	player.CharacterAppearance = "_char"	
	if not test then visit:SetUploadUrl("")end
...
pcall(function() game:SetVideoInfo('<?xml version="1.0"?><entry xmlns="http://www.w3.org/2005/Atom" xmlns:media="http://search.yahoo.com/mrss/" xmlns:yt="http://gdata.youtube.com/schemas/2007"><media:group><media:title type="plain"><![CDATA[ROBLOX Place]]></media:title><media:description type="plain"><![CDATA[ For more games visit _burl]]></media:description><media:category scheme="http://gdata.youtube.com/schemas/2007/categories.cat">Games</media:category><media:keywords>ROBLOX, video, free game, online virtual world</media:keywords></media:group></entry>') end)
