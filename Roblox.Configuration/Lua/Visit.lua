-- TODO: handle the Edit.lua and Visit.lua and Studio.lua and PlaySolo.lua stuff
...
pcall(function() game:GetService("InsertService"):SetFreeModelUrl("_burl/Game/Tools/InsertAsset.ashx?type=fm&q=%s&pg=%d&rs=%d") end) -- Used for free model search (insert tool)
pcall(function() game:GetService("InsertService"):SetFreeDecalUrl("_burl/Game/Tools/InsertAsset.ashx?type=fd&q=%s&pg=%d&rs=%d") end) -- Used for free decal search (insert tool)
...
pcall(function() game:GetService("Players"):SetBuildUserPermissionsUrl("_burl//Game/BuildActionPermissionCheck.ashx?assetId=0&userId=%d&isSolo=true") end)
...
function doVisit()
	...
	player.CharacterAppearance = "_burl/Asset/CharacterFetch.ashx?userId=1&placeId=0"
	...
	if not inStudio and false then
		...
		visit:SetPing("_burl/Game/ClientPresence.ashx?version=old&PlaceID=0", 120)
...
if success then
	...
	if not inStudio then
		if false then
			game:HttpPost("_burl/Error/Lua.ashx?", "Visit.lua: " .. err)
		end
	end
end
